...
标题块的 `alignment` 表示源文档中的语义对齐方式。Adapter 应从 Provider 的版面几何信息通用推导；缺少可靠几何信息时使用 `left`，不得根据标题文字或文件名猜测。标题 `text` 可包含由可靠 OCR 行内几何恢复出的有效空白，Renderer 通过保留空白还原标题内部的相对位置。Renderer 使用语义对齐进行响应式排版，不消费 Provider 坐标。
