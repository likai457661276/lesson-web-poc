...
def _upload(filename: str) -> UploadFile:
    return UploadFile(filename=filename, file=BytesIO(b"fake-bytes"))
...
@pytest.mark.parametrize("filename", ["lesson.doc", "lesson.docx"])
async def test_create_job_rejects_word_documents(tmp_path: Path, filename: str) -> None:
